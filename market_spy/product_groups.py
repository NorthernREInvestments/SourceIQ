...
def _match_sourcing(selling_items: list[dict], sourcing_items: list[dict], limit: int = 3) -> list[dict]:
    ...
    for src in sourcing_items:
        ...
        enriched = enrich_sourcing_pricing(dict(src))
        landed = landed_cost(enriched)
        if landed is None:
            continue
        scored.append((landed, score, enriched))
    scored.sort(key=lambda row: (row[0], -row[1]))
    suppliers = []
    for landed, _score, item in scored[:limit]:
        moq = int(item.get("moq") or 1)
        unit = item.get("unit_price") or item.get("price")
        bulk = item.get("bulk_price") or item.get("price")
        suppliers.append({
            "platform": item.get("source", "Supplier"),
            "unit_price": unit,
            "unit_price_display": _format_usd(unit),
            "bulk_price": bulk,
            "bulk_price_display": _format_usd(bulk),
            "moq": moq,
            "shipping_usa": item.get("shipping_usa"),
            "shipping_display": _format_usd(item.get("shipping_usa")),
            "landed_cost": landed,
            "landed_display": _format_usd(landed),
            "url": item.get("url", ""),
        })
    return suppliers
...
def _serialize_group(
    ...
    *,
    ...
    if len(selling) < min_products and min_products > 1:
        pass  # singles allowed as individual cards
    ...
    source_costs = [s["landed_cost"] for s in suppliers if s.get("landed_cost") is not None]

    sell_min, sell_max = min(sell_prices), max(sell_prices)
    src_min = min(source_costs) if source_costs else None
    src_max = max(source_costs) if source_costs else None

    margins = []
    for sp in sell_prices:
        for sc in source_costs or [None]:
            if sc is None:
                continue
            m = _margin_pct(sp, sc)
            if m is not None:
                margins.append(m)
    if not margins and source_costs:
        mid_sell = (sell_min + sell_max) / 2
        margins = [_margin_pct(mid_sell, sc) for sc in source_costs]
    ...
    margin_tier = _margin_tier(margin_mid)
    ...
    if suppliers:
        ...
        best_source = {
            ...
            "landed": best["landed_cost"],
            "landed_display": best["landed_display"],
        }

    name = _display_type_name(selling, search_term)
    primary_source = Counter(i.get("source") for i in selling).most_common(1)[0][0]
    ...
    return {
        ...
        "sell_source_label": f"({primary_source} sold avg)",
        "source_min": src_min,
        "source_max": src_max,
        "source_range_display": _format_range(src_min, src_max) if src_min else "—",
        "best_source": best_source,
        "margin_min": margin_min,
        "margin_max": margin_max,
        "margin_range_display": (
            f"{margin_min:.0f}% — {margin_max:.0f}%"
            if margin_min is not None and margin_max is not None and margin_min != margin_max
            else (f"{margin_min:.0f}%" if margin_min is not None else "—")
        ),
        "margin_mid": margin_mid,
        "margin_tier": margin_tier,
        "margin_tier_lower": margin_tier.lower(),
...
def build_product_groups(
    ...
    *,
    ...
    groups.sort(key=lambda g: (-(g.get("margin_mid") or 0), -(g.get("product_count") or 0)))
    return groups
